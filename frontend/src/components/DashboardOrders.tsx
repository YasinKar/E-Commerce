...
type DashboardOrdersProps = {
    status: 'delivered' | 'processing' | 'out_for_delivery' | 'pending_payment'
}

const DashboardOrders: React.FC<DashboardOrdersProps> = async ({ status }) => {
            ...
            <ul className="mb-8 flex flex-wrap items-center justify-center gap-2 md:justify-start" role="tablist" >
                <li role="presentation">
                    <Link
                        ...
                        className={`rounded-lg px-6 py-2 text-sm ${!status ? 'bg-sky-500 text-white' : 'border border-sky-500 hover:bg-sky-100'}`}
                    >
                        All
                    </Link>
                </li>
